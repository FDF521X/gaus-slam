        ...
        bool use_sa)
{
	...
	if (inside)
	{
		...
		if (use_sa) {
			out_others[pix_id + DISTORTION_OFFSET * H * W] = D2 - 2 * median_depth * D + (1-T) * median_depth * median_depth;
		}
		else {
...
__device__ float computeCamMaskFromSH(int deg, int W, int H, float2 pixf, const float* camsh, bool& clamped)
{
	// The implementation is loosely based on code for 
	// "Differentiable Point-Based Radiance Fields for 
	// Efficient View Synthesis" by Zhang et al. (2022)
	float f = max(W, H) / 2;
	glm::vec3 dir = glm::vec3(pixf.x, pixf.y, f);
	dir = dir / glm::length(dir);
	deg = 3;
	const float* sh = camsh;
